# prototyping for KMC lab practical
...
class Droplet(Substrate):
    ...
    def __init__(self, delta_stack, energy_ev_stack, energy_bind, delta_climb):
        super().__init__(delta_stack, energy_ev_stack)
...
class Surface(Substrate):
    ...
    def __init__(self, sizes, delta_surface, energy_ev_surface):
        super().__init__(delta_surface, energy_ev_surface)
    ...
    def __iter__(self):
            self.adsorbate.values())  # when an iterator-type object is needed, the list of values in the surface "dict" are given

    def add_part(self, s, part):  # add a particle given a site obj
        if s.occ == self.max_lz: return None
        s.occ += 1
...
class _Site(Substrate):
    ...
    def __init__(self, surf, coords):
        super().__init__(surf.de_diff, surf.de_desorb)
...
class _Coordinate(object):
    '''Position (given periodic boundary conditions) of a _Site object embedded on a Surface object,
    defined in term of non-periodic cartesian coordinates'''
